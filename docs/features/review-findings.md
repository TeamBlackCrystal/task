...
|---|---|
| `project_id` | 対象プロジェクト |
| `integration_id` / `repo_owner` / `repo_name` | ラウンドを出した時点の GitHub 連携と、その連携先リポジトリの控え。連携が無ければ空（PR 番号だけの記録になる） |
| `pr_number` | PR 番号 |
...
| `GET  /v1/tenants/{t}/projects/{p}/reviews/summary?pr=618` | PR 単位の集計: 重大度 × 状態の件数、ラウンド数、最新ラウンドの `head_sha`、「マージ可否」 |
...
# 照合する HEAD を明示する（CI では checkout した SHA を渡す）
task review summary --project TASK --pr 618 --head "$GITHUB_SHA"
