...
- **`rejected` への遷移と `rejected → open` は、その指摘を出したラウンドの作成者だけ**。
  ...
  常設する。§5 / §7）。除名・退会で作成者が消えると、誤った High を取り下げる主体が
...
- **繰り延べを繰り返しても、自動起票するタスクは指摘 1 件につき 1 つ**。`deferred → open`
  でクローズしたあと再び `deferred` にしたら、新しく起票せず `deferred_task_id` の
  タスクを再オープンして使い回す。毎回起票すると、往復のたびに `seq_id` と通知を消費して
  タスク一覧が同じ内容で埋まる
...
- **1 PR × 1 プロジェクトに 1 本**。HTML マーカーに**プロジェクト ID を含める**
  （例: `<!-- koyori-review-summary:{project_id} -->`）で自分のコメントを特定し、
  以後は**同じコメントを編集**して更新する。
...
- **状態遷移の操作**: fixed / verified / deferred / rejected に加えて、**戻り遷移**
  ...
  rejected は指摘を出した本人にだけ出す）。§3 の表と 1:1 で対応させる
- **マージ可否の即答**: **リポジトリ未確定 / 未レビュー / マージ不可（残数つき）/ マージ可**
  の 4 つを出し分ける（§5 と同じ規則）。「リポジトリ未確定」は GitHub 連携が無い状態で、
  **ゲートとして扱ってよい表示ではない**——連携を外すと集計の視界が空になり、
  空のラウンド 1 本で「可」を作れるため。CLI だけで塞いでも、人間の主経路である
  画面が素通しならゲートの半分が無いのと同じになる
...
- 2026-08-28: 連携なしのゲート拒否を**画面にも要求する**。CLI にだけ実装すると、
  人間の主経路（§1）が素通しになる。マージ可否の表示は「リポジトリ未確定」を含む 4 値
