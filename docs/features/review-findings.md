...
> **予定（[GitHub↔タスク連携](/features/tasks/github-tasks) §2）**: `integration_id` / `repo_owner` /
> `repo_name` / `pr_number` / `pr_title` / `pr_author` は PR の実体表 `github_pull_requests` へ移り、
...
- 2026-09-02: PR を実体表（`github_pull_requests`）へ切り出し、`reviews` は FK で参照する方針を [GitHub↔タスク連携](/features/tasks/github-tasks) 側に置いた。レビュー指摘 → PR → タスクと辿るため。実装は同仕様の分割 1 で行う
