...
- **`rejected` への遷移と `rejected → open` は、その指摘を出したラウンドの作成者だけ**。
  ただし**作成者がそのプロジェクトの利用者でなくなっている場合に限り、テナントオーナーが
  代行できる**（誰が代行したかは遷移履歴に残る）。除名・退会で作成者が消えると、
  誤った High を取り下げる主体が永久に居なくなり、直していないものを
  `fixed → verified` と記録するしかなくなる——監査記録に嘘を書かせないための例外。
...
| `GET  /v1/tenants/{t}/projects/{p}/reviews/summary?pr=618` | PR 単位の集計: 重大度 × 状態の件数、ラウンド数、最新ラウンドの `head_sha`、集計対象のリポジトリ、「マージ可否」 |
...
- 集計は**どのリポジトリを見た結果か**（`repository`。連携が無ければ空）も返す。
  ...
  **リポジトリが確定しない集計をゲートとして通さない**のは呼び出し側の役目（§6）
...
- **マージ可否の即答**: 未レビュー / マージ不可（残数つき）/ マージ可 の 3 つを出し分ける
  （§5 と同じ規則。ラウンド 0 件を「可」と出さない）。レビューした commit も添える
  ——手元の HEAD と見比べれば、レビュー後に積まれたコミットに気づける
...
- 取り下げの代行: 作成者がプロジェクトの利用者でなくなった指摘は、テナントオーナーが
...
- 2026-08-26: 取り下げは、作成者がプロジェクトの利用者でなくなった場合に限り
