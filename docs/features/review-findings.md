...
履歴として残る。採番には `UNIQUE (project_id, pr_number, round)` を張り、同じ PR へ
ほぼ同時にラウンドが確定しても番号が重ならないようにする（採番から挿入までを
プロジェクト行のロックで直列化し、制約は最後の防波堤として残す）。番号が重なると
R1, R2, … の表示と「どの head を見た判断か」の対応が崩れる。
...
- **`deferred` へ遷移できるのは `low` / `nit` の指摘だけ**。`high` / `medium` の繰り延べは
  ...
  迂回できてしまう。`rejected` は「レビュー側のみ」で守られているのに `deferred` だけ
  素通しになる非対称も、ここで塞ぐ
...
- 更新契機: ラウンドの作成時と指摘の状態遷移時（apalis ジョブで非同期。ペイロードに
  機微情報を載せない既存規約に従う）
- 同一 (project, pr) の更新要求は**1 本に合流させる**。要求ごとにジョブを積むと、20 件の
  指摘を順に `fixed` にしただけで同じコメントへ 20 回書き込みに行き、GitHub の
  secondary rate limit に当たる。投入前に「更新待ち」の印を立て、既に立っていれば積まない。
  ジョブは実行開始時にその印を落としてから最新状態を読み直すので、合流された更新も
  次の 1 回に必ず含まれる。印には**TTL** を置く。ジョブが再試行を使い切って死ぬと
  印だけが残り、「立っていれば積まない」規則で以後の遷移が 1 件も積まれなくなるため
  （TTL が切れれば次の遷移で自然に再開する）
- 合流に加えて、**同一 (project, pr) のジョブ実行そのものを直列化する**。合流は
  ジョブの本数を減らすだけで、同時に走ることは止められない。並行して走ると、
  先に古い状態を読んだジョブの書き込みが後から着き、コメントが巻き戻ったまま
  次の遷移まで直らない。ロックを取れなかったジョブは印を落とさずにエラーで戻し、
  apalis の再試行で取り直す（そのとき最新状態を読み直す）
...
- 2026-08-26: ラウンドの採番は `UNIQUE (project_id, pr_number, round)` + 行ロックで直列化する
- 2026-08-26: 要約更新ジョブは同一 (project, pr) で合流させる（GitHub の secondary rate limit 対策）。
  ...
  ロック・印とも TTL 付きで、解放はトークン照合つき（担い手が落ちても詰まらせない）
