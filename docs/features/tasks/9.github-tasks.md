...
| `PullRequest / Closed { merged: true }` | `state → merged`、`merged_at`。`closes = true` のリンク先で未完了のタスクを完了ステータスへ遷移し、Automation `github_pr_merged` トリガーを発火（トリガー名はホスト中立に `pull_request_merged` へ改名し、旧名は別名として受ける） |
...
- `Closed { merged: true }`: `closes = true` のタスクだけ完了する / 既に完了のタスクは遷移しない / `closes = false` は触らない
