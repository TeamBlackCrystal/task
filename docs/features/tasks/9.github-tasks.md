...
SELECT gen_random_uuid(), project_id,
       ...
       COALESCE(max(pr_title), ''), COALESCE(max(pr_author), ''), 'unknown',
...
```text
受信時:
  署名検証 → 失敗なら 401 を返して終わり（受信記録もジョブも作らない）
...
`PullRequest` の **すべての action** は、action 固有の処理より先に、適用対象のスナップショット
（下記の比較で決める）から `forge_pull_requests` を UPSERT する。少なくとも `title` /
`author_handle` / `state` / `head_sha` / `html_url`（および `merged_at` があればそれ）と
`host_updated_at` を更新し、PR行がまだ無い場合もここで作成する。これにより、`Opened` を受信していない既存PRでも後続の処理が既存行の存在に依存しない。
...
|---|---|
| `>` | イベントのペイロード |
| `<` | 何も適用しない。UPSERT も action 固有の処理（リンクの再構築、状態遷移、コミット集合の同期、マージ時のタスク完了）も行わず、受信記録だけ残して捨てる |
...
後に届いたときに `title`・本文由来のリンク・`head_sha`・コミット集合が古い内容へ戻る。
...
取り直したスナップショットでも **`state` が `merged` の行は `open` / `closed` へ戻さない**。
マージは取り消せない操作で、ホスト側にも「マージ済みを開き直す」遷移が無い。
`Reopened` が正当に `open` へ戻すのは `closed` からだけ。

action 固有の処理も、UPSERT と同じ内容（`>` はペイロード、`=` は取り直したスナップショット）を
入力にする。`Edited` はそのタイトル・本文から `title` / `body` のリンクを再構築し、`Closed` は
タイトル・本文・headブランチから `title` / `body` / `branch` のリンクを再構築してから、
...
- webhook の署名検証: 署名が不正なリクエストは 401 で、`forge_webhook_deliveries` の行もジョブも作られない。その後に同じ `delivery_id` の正当なイベントが届いたら初回として処理される（先取りで本物が重複扱いにならない）
...
- 配送順の逆転: `Closed { merged: true }` を処理した後に、それより古い `updated_at` の `Edited` / `Opened` / `Synchronize` が届いても、`state` が `merged` のまま・`title` と `head_sha` が巻き戻らない・リンクが再構築されない。対照として、新しい `updated_at` のイベントはペイロードどおり適用される。`host_updated_at` が `NULL` の既存行（CLI の `review submit` だけで作られた行）に最初のイベントが届いたときも適用される
- 同じ秒のイベント: 同一 `updated_at` の `Edited` / `Synchronize` / `Closed { merged: true }` を逆順（`Closed` → `Synchronize` → `Edited`）で処理しても、`title` / `body` 由来のリンク / `head_sha` / コミット集合が古いペイロードの内容に戻らず、すべて PR 取得 API が返す現在のスナップショットと一致する（ペイロードにしか無い値が入らないこと）。同値のときだけ取得 API が呼ばれ、`>` と `<` では呼ばれない。取得 API が 5xx を返したときは PR 行もリンクも一切変わらずジョブが再試行される
...
- `reviews` の付け替え: `pr_title` と `pr_author` が両方 NULL の既存ラウンド（API 経由で作られたラウンド）を含めても移行が完走し、その PR 行の `title` / `author_handle` が空文字列になる。既存ラウンド（連携前の空文字列リポジトリを含む）が移行後も同じ `pr_number` / `round` で一覧に出る。連携先差し替え後の同番号 PR が別の PR 行になる。`host_url` が違う同名リポジトリ（github.com と自前 Forgejo の `org/app`）が別の PR 行になる
...
- 2026-09-06: `updated_at` が同値のイベントは、ペイロードではなくホストの PR 取得 API で取り直したスナップショットを適用する（取得できなければジョブを再試行）。`merged` を戻さない防御だけでは、同じ秒の古い `Edited` / `Synchronize` が後着したときに `title`・本文由来のリンク・`head_sha`・コミット集合が巻き戻るため
- 2026-09-06: 受信記録の INSERT は署名検証を通した後に行うと明記（署名検証前だと、誰でも任意の `delivery_id` で永続表を増やせ、配信 ID を先取りされると正規のイベントが重複として捨てられる）
- 2026-09-06: `reviews` から PR 行を起こす移行で `title` / `author_handle` を `COALESCE(max(...), '')` にする（移行元は NULLABLE で両方 NULL のラウンドがあり、`INSERT ... SELECT` は NULL を明示的に入れるため NOT NULL DEFAULT '' の列で移行が止まる）
