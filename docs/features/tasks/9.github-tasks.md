...
|---|---|
| `>` | イベントのペイロード |
| `<` | PR スナップショットを適用しない。UPSERT・リンクの再構築・状態遷移・マージ時の副作用は行わない。ただし元の action が `Opened` / `Synchronize` なら、現在の一覧を正とするコミット集合の同期は行う |
...
`open` / `closed` でも戻さない。リンクの再構築後、PR 行の保存済み状態が `merged` 以外から
実効状態 `merged` へ変わるときだけ、`bool_or(closes)` が true の未完了タスクを完了させ、
Automation `github_pr_merged` トリガーを発火する（トリガー名はホスト中立に
`pull_request_merged` へ改名し、旧名は別名として受ける）。同じスナップショットの再処理では
マージ時の副作用を繰り返さない。
...
限らないためである。また、マージ時にこれを省くと、本文が `Closes ENG-42` で開かれた PR から後に参照が消え、その
...
| `PullRequest / Closed { merged: true }` | **共通処理**。実効状態が `merged` へ変わる場合だけ、共通処理内でタスク完了と Automation を実行 |
...
その PR に属する `forge_pull_request_commits` を **一覧と一致させる**。この API が返すのは
呼び出した時点の現在の一覧なので、古いイベントが後着してもコミット集合は巻き戻らない。
...
- 配送順の逆転: `Closed { merged: true }` を処理した後に、それより古い `updated_at` の `Edited` / `Opened` / `Synchronize` が届いても、`state` が `merged` のまま・`title` と `head_sha` が巻き戻らない・リンクが再構築されない。古い `Opened` / `Synchronize` でもコミット一覧 API は呼ばれ、コミット集合だけはその現在の一覧に同期する。対照として、新しい `updated_at` のイベントはペイロードどおり適用される。`host_updated_at` が `NULL` の既存行（CLI の `review submit` だけで作られた行）に最初のイベントが届いたときも適用される
...
- 同じ秒の状態遷移: 同一 `updated_at` の `Closed { merged: false }` と `Reopened` を両方の配送順で処理し、いずれも最終的な `state` とリンク集合が PR 取得 API の現在のスナップショットと一致する。後着した元 action の状態遷移を重ねて実行しない。取得結果が `merged` なら、保存済み状態が非 `merged` のときだけタスク完了と Automation が 1 回実行される
...
- 2026-09-07: PR イベントの適用条件に世代（`forge_pull_requests.host_updated_at` と正規化イベントの `updated_at`）を導入。古いイベントは PR 行・リンク・状態を変更しない。一方、`Opened` / `Synchronize` のコミット集合は世代に関係なく現在の一覧 API と同期する（レビュー指摘: 配送順・実行順が保証されないため、無条件 UPSERT だと新しい状態が巻き戻るが、コミット一覧はイベントのペイロードではないため巻き戻らない）
- 2026-09-07: 適用対象になったすべての PR action で `title` / `body` / `branch` のリンクを最新集合へ再構築する（レビュー指摘: `Opened` より先に `Synchronize` / `Reopened` / 未マージの `Closed` を初回観測するとリンクされない）。マージ時はこの再構築後に `bool_or(closes)` を評価する
- 2026-09-07: `updated_at` が同値のイベントは、ペイロードではなくホストの PR 取得 API で取り直したスナップショットを適用する（取得できなければジョブを再試行）。`title`・本文由来のリンク・`head_sha` の巻き戻しを防ぐだけでなく、元 action の状態遷移を実行せず、取得結果から実効状態とマージ時の副作用を決める（レビュー指摘: 同秒の `Closed` と `Reopened` は action を後から重ねると現在状態を巻き戻せる）
