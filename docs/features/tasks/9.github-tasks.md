...
`html_url`（および `merged_at` があればそれ）を更新し、PR行がまだ無い場合もここで作成する。
これにより、`Opened` を受信していない既存PRや、配送順が前後したPRでも後続の処理が既存行の存在に依存しない。

その後、`Edited` は最新のタイトル・本文から `title` / `body` のリンクを再構築し、`Closed` は
最新のタイトル・本文・headブランチからリンクを確定してから、マージ時の
`bool_or(closes)` によるタスク完了処理を行う。再解析の対象外である `manual` のリンクは残す。
...
| `PullRequest / Closed { merged: true }` | **共通前処理でPRスナップショットをUPSERT**し、タイトル・本文・headブランチ名を再解析してリンクを確定する。その後 `state → merged`、`merged_at`。リンク行の `bool_or(closes)` が true のタスクのうち未完了のものを完了ステータスへ遷移し、Automation `github_pr_merged` トリガーを発火（トリガー名はホスト中立に `pull_request_merged` へ改名し、旧名は別名として受ける） |
| `PullRequest / Closed { merged: false }` | **共通前処理でPRスナップショットをUPSERT**した後、`state → closed` |
| `PullRequest / Reopened` | **共通前処理でPRスナップショットをUPSERT**した後、`state → open` |
