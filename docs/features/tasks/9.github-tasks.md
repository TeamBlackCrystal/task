...
| `pull_request_id` | UUID | NULLABLE, FK→forge_pull_requests SET NULL | その PR に含まれるコミットとして観測されたとき |
| `created_at` | TIMESTAMPTZ | NOT NULL DEFAULT now() | |
...
CREATE INDEX idx_forge_commits_pr ON forge_commits(pull_request_id);
...
| `PullRequest / Opened` | `forge_pull_requests` を UPSERT（title / author / state / head_sha / html_url）。タイトル + 本文 + head ブランチ名をパースしてリンク |
| `PullRequest / Edited` | タイトル・本文を再解析。`source in (title, body)` のリンクを作り直す（`manual` / `branch` は触らない） |
| `PullRequest / Synchronize` | `head_sha` を更新。PR のコミット一覧 API で `forge_commits.pull_request_id` を埋める |
...
`KEY-N` が残っていれば、次の Edited / Synchronize の再解析で `title` / `body` 由来の行が復活する。
