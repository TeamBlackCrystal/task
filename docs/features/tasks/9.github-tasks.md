...
| `PullRequest / Synchronize` | `head_sha` を更新。PR のコミット一覧 API の結果で `forge_pull_request_commits` を UPSERT する（既存の PR ↔ コミット行は消さない） |
...
- `Synchronize`: 同じコミットが 2 つの PR のコミット一覧に出たとき、両方の PR に結ばれ、後から処理した PR が先の関連を消さない
