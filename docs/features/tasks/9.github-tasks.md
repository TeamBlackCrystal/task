...
積むときは `INSERT ... ON CONFLICT (dedupe_key) DO NOTHING RETURNING id` で、行が返ったときだけ
「初回」とみなす。**現在のリンク行を数えて初回を判定しない。** リンクを外す DELETE は
...
| `GET /forge/pull-requests/{pull_request_id}` | パスのプロジェクトのタスク閲覧権限 | `read:task` |
