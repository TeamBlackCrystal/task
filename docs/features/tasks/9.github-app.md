...
6. installation の所有者を確認する
   - code を GitHub App の Client ID / Client secret でユーザーアクセストークンに交換し、
     GET /user/installations に installation_id が含まれるかを見る
   - 含まれない・code が無効・code そのものが無い → 設定画面へ
     ?...&github_error=installation_forbidden で戻す（対処は自分がアクセスできる
     アカウント / Organization へ入れ直すこと）
...
| callback への他人の `installation_id` の差し込み | インストール時のユーザー認可で受け取った `code` をユーザーアクセストークンに交換し、`GET /user/installations` に当該 `installation_id` が含まれることを確認。含まれなければ `installation_forbidden` で拒否（User インストールと Organization インストールのどちらも同じ経路で確かめられる） |
