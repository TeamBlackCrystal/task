...
7. installation が見えるリポジトリを列挙する
   - 0 件 → 連携先を選びようがないので 400（GitHub 側で選び直してもらう）
   - 1 件 → そのまま連携先に決める（8 へ）
...
8. トークンを AES-256-GCM で暗号化して DB に保存（project_id で github_integrations を UPSERT）

9. 設定完了 → フロントの設定画面へリダイレクト
```
...
| `GET` | `/v1/tenants/{tid}/projects/{pid}/github/repositories` | 選択トークンに紐づく installation のリポジトリ一覧（トークンは消費しない） |
...
`GET /github/repositories?select_token=xxx` レスポンス:
...
`GET /github/repositories` の結果を並べて 1 件選ばせ、`POST /github/connect` で確定する。
...
「連携する」を押し直したときの state 束縛に使う（新規インストール扱いの鮮度チェックで
弾かれて詰むのを防ぐ）。連携が確定したときと連携解除のときは消す。GitHub 側で
消えているインストールを指していた場合も、束縛に使わず新規扱いに戻す
（残したままだと入れ直したインストールを逆に弾いてしまう）。
