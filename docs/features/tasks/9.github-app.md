...
**この待ち時間は画面のローカル状態にすぎない。** リロード・画面の再訪・別タブでは消えるし、サーバー側に重複の防止は無く、
`POST /github/import` は連携の有無だけを見てジョブを積む。実際に重複を止めるには、ジョブを積む側で
（プロジェクト単位のロック、または同じプロジェクトの取り込みジョブが待機中ならスキップする等の）対策が要る。
