...
管理者が「除名したのにまだ入れる」と驚かないよう、除名の応答（200）は残る明示 ACE を `remaining_explicit_projects`（project_id、key、name、role）で名指しする。
閉め出したい場合は、そこに挙がったプロジェクトのメンバーからも外す。
空なら、その人はもうそのテナントのどこにも入れない。
...
| 除名は継承を外すだけで、明示 ACE を消さない | `tenant_members::remove_member` は `project_members` の行を消さず、残る行を応答で名指しする |
...
閉め出したい場合はプロジェクトメンバーからも外す（除名の応答が対象を名指しする）。
...
| テナント除名 | 200（残る明示 ACE を同梱。204 ではない） |
